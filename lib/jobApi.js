...
// Mock jobs for development/fallback — clearly labeled, with Google search links so users can still find real postings
function getMockJobs(keywords, filters = {}, source = 'mock') {
  const keywordStr = Array.isArray(keywords) ? keywords.join(', ') : keywords;
  const loc = filters.location || 'Remote';
  
  const roles = [
    { title: `${keywordStr} Developer`, salary: '$90k-140k', company: 'Sample Corp (Mock)', desc: `Looking for a skilled ${keywordStr} developer to join our team. This is a sample listing to show what results look like.` },
    { title: `Senior ${keywordStr} Engineer`, salary: '$120k-180k', company: 'Tech Holdings (Mock)', desc: `Senior-level position requiring deep ${keywordStr} expertise. Sample listing for demonstration.` },
    { title: `${keywordStr} Architect`, salary: '$130k-190k', company: 'Enterprise Solutions (Mock)', desc: `Architecture role focused on ${keywordStr} systems and design. Sample listing for demonstration.` },
    { title: `Junior ${keywordStr} Developer`, salary: '$60k-90k', company: 'StartupXYZ (Mock)', desc: `Entry-level ${keywordStr} position perfect for recent graduates. Sample listing.` },
    { title: `${keywordStr} Lead`, salary: '$140k-200k', company: 'Global Tech (Mock)', desc: `Team lead role overseeing ${keywordStr} projects and mentoring developers. Sample listing.` },
  ];
  ...
  return roles.map((role, idx) => ({
    ...
    location: loc,
    salary: role.salary,
    jobDescription: role.desc,
    jobUrl: `https://www.google.com/search?q=${encodeURIComponent(role.title + ' jobs ' + loc)}`,
    postedDate: new Date().toISOString(),
    employmentType: 'FULLTIME',
    source: 'mock',
    tags: Array.isArray(keywords) ? keywords : [keywords]
  }));
}
