 ...
 * Handles learning path integration, course discovery, and skill gap analysis
...
async function generateLearningPath(resumeStructured, targetRole, filters = {}) {
  try {
    ...
    // Build flat courses array from timeline phases for frontend rendering
    const allCourses = [];
    if (learningPath.timeline) {
      Object.values(learningPath.timeline).forEach(phase => {
        if (phase.courses && Array.isArray(phase.courses)) {
          allCourses.push(...phase.courses);
        }
      });
    }
    learningPath.courses = allCourses;

    // Build steps array from timeline phases for frontend rendering
    const steps = [];
    if (learningPath.timeline) {
      Object.values(learningPath.timeline).forEach(phase => {
        steps.push({
          title: `${phase.focus}: ${(phase.skills || []).join(', ') || 'General'}`,
          description: `Focus on ${phase.focus.toLowerCase()} skills`,
          duration: phase.duration,
        });
      });
    }
    // Add project steps
    if (learningPath.projects) {
      learningPath.projects.forEach(p => {
        steps.push({
          title: p.title,
          description: p.description,
          duration: p.timeInvest,
        });
      });
    }
