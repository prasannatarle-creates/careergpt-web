/**
 * Analytics Dashboard
 * Tracks user engagement metrics, funnel analysis, and usage statistics
 */
...
async function analyzeFunnel(db) {
  try {
    ...
    const [signups, resumeAnalyzed, interviewsStarted, jobOffers] = await Promise.all([
      ...
      db.collection('saved_jobs').countDocuments({ status: 'offered' }), // Track REAL offers from saved jobs
    ]);
    ...
    const funnelStages = {
      ...
      offersReceived: jobOffers, // Track actual offer events, not estimates
    };
...
async function getUserSegmentation(db, segmentType = 'role') {
  try {
    ...
    if (segmentType === 'role') {
      ...
      // Segment by location from user profiles and career paths
      const locationSegments = {};
      const careerPaths = await db.collection('career_paths').find({}).toArray();
      careerPaths.forEach(path => {
        const loc = path.input?.location || 'Not Specified';
        locationSegments[loc] = (locationSegments[loc] || 0) + 1;
      });
      // Also check job match searches
      const jobMatches = await db.collection('job_matches').find({}).toArray();
      jobMatches.forEach(jm => {
        const loc = jm.input?.location || 'Not Specified';
        locationSegments[loc] = (locationSegments[loc] || 0) + 1;
      });
      // If no data, show informative message
      if (Object.keys(locationSegments).length === 0) {
        locationSegments['No Data Yet'] = 0;
      }
      ...
      return {
        ...
        totalUsers: users.length,
        distribution: 'Geographic distribution based on user career path and job search locations',
      };
    }
